...
pathDat <- R.utils::Arguments$getWritablePath("Data/SimDat_Noise")
pathMeth <- R.utils::Arguments$getWritablePath("Output/SimDat_Noise")

S <- 50

n.sample <- c(rep(150, 4))
...
p.DMP <- c(rep(0.1, 4))
delta.methyl <- c(rep(2, 4))
delta.expr <- c(rep(2, 4))
delta.protein <- c(rep(2, 4))
noise.methyl <- c(0.5, 1, 2, 3)
noise.expr <- c(0.5, 1, 2, 3)
noise.protein <- c(0.5, 1, 2, 3)
...
ARI_dat$benchmark <- ARI_dat$benchmark %>% factor(levels = paste0("Benchmark", 1:4))
...
NMI_dat$method <- NMI_dat$method %>% factor(levels = c("LRAcluster", "SNF", "CIMLR", "Mocluster",
                                                       "intNMF", "MCIA", "iClusterPlus", "PINSPlus"))
NMI_dat$benchmark <- NMI_dat$benchmark %>% factor(levels = paste0("Benchmark", 1:4))
...
F1_dat$method <- F1_dat$method %>% factor(levels = c("LRAcluster", "SNF", "CIMLR", "Mocluster",
                                                     "intNMF", "MCIA", "iClusterPlus", "PINSPlus"))
F1_dat$benchmark <- F1_dat$benchmark %>% factor(levels = paste0("Benchmark", 1:4))
...
Noise_data <- rbind(ARI_dat_mean, NMI_dat_mean, F1_dat_mean) %>% 
  mutate(Index = factor(Index, levels = c("ARI", "NMI", "F1-score")))

Noise_data_wide <- Noise_data %>% mutate(mean = sprintf("%.3f", mean) %>% as.numeric)
Noise_data_wide <- Noise_data_wide %>% pivot_wider(names_from = method, values_from = mean)

g_noise_line <- Noise_data %>% ggplot(aes(x = benchmark, y = mean, group = method, color = method, shape = method)) +
  geom_point(size = 2.5) + 
  geom_line(linewidth = 1) + 
  scale_color_manual(values  = MetBrewer::met.brewer("VanGogh2", 8)) +
  scale_shape_manual(values = c(1:8)) + 
  scale_x_discrete(labels = as.character(c(0.5, 1, 2, 3))) + 
  facet_wrap(. ~ Index, ncol = 3, labeller = label_wrap_gen(multi_line = T)) +
  theme_bw() + xlab("Noise") + ylab("") + 
