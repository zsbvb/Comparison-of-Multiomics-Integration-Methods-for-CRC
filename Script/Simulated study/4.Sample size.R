...
pathDat <- R.utils::Arguments$getWritablePath("Data/SimDat_Sample")
pathMeth <- R.utils::Arguments$getWritablePath("Output/SimDat_Sample")

S <- 50

n.sample <- seq(50, 500, by = 50)
cluster.sample.prop = list(B1 = c(0.2, 0.3, 0.5), B2 = c(0.2, 0.3, 0.5), B3 = c(0.2, 0.3, 0.5), 
                           ...
                           B7 = c(0.2, 0.3, 0.5), B8 = c(0.2, 0.3, 0.5), B9 = c(0.2, 0.3, 0.5), 
                           B10 = c(0.2, 0.3, 0.5))
n.clust <- sapply(1:length(n.sample), function(x) length(cluster.sample.prop[[x]]))
p.DMP <- c(rep(0.1, 10))
delta.methyl <- c(rep(2, 10))
delta.expr <- c(rep(2, 10))
delta.protein <- c(rep(2, 10))
noise.methyl <- c(rep(FALSE, 10))
noise.expr <- c(rep(FALSE, 10))
noise.protein <- c(rep(FALSE, 10))
...
F1_dat_mean <- F1_dat %>% 
  ...
  mutate(Index = "F-score")

Sim_sample_res <- rbind(ARI_dat_mean, NMI_dat_mean, F1_dat_mean)
Sim_sample_res$Index <- factor(Sim_sample_res$Index, levels = c("ARI", "NMI", "F-score"))

Sim_sample_res_wide <- Sim_sample_res %>% mutate(mean = sprintf("%.3f", mean) %>% as.numeric)
Sim_sample_res_wide <- Sim_sample_res_wide %>% pivot_wider(names_from = method, values_from = mean)

library(ggplot2)
library(ggalt)

g_sample_line <- Sim_sample_res %>% 
  ...
  scale_x_discrete(labels = as.character(seq(50, 500, by = 50))) + 
