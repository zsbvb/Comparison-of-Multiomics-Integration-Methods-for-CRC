...
pathDat <- R.utils::Arguments$getWritablePath("Data/SimDat_Clust")
pathMeth <- R.utils::Arguments$getWritablePath("Output/SimDat_Clust")

S <- 50

n.sample <- c(rep(150, 5))
cluster.sample.prop = list(B1 = c(0.4, 0.6), B2 = c(0.2, 0.3, 0.5), B3 = c(0.1, 0.2, 0.2, 0.5), 
                           B4 = c(0.1, 0.1, 0.2, 0.2, 0.4), B5 = c(0.1, 0.1, 0.1, 0.2, 0.2, 0.3))
n.clust <- sapply(1:length(n.sample), function(x) length(cluster.sample.prop[[x]]))
p.DMP <- c(rep(0.1, 5))
delta.methyl <- c(rep(2, 5))
delta.expr <- c(rep(2, 5))
delta.protein <- c(rep(2, 5))
noise.methyl <- c(rep(FALSE, 5))
noise.expr <- c(rep(FALSE, 5))
noise.protein <- c(rep(FALSE, 5))
...
ARI_dat$benchmark <- ARI_dat$benchmark %>% factor(levels = paste0("Benchmark", 1:5))
ARI_dat_mean <- ARI_dat %>% group_by(benchmark, method) %>% 
...
NMI_dat$method <- NMI_dat$method %>% factor(levels = c("LRAcluster", "SNF", "CIMLR", "Mocluster",
                                                       "intNMF", "MCIA", "iClusterPlus", "PINSPlus"))
NMI_dat$benchmark <- NMI_dat$benchmark %>% factor(levels = paste0("Benchmark", 1:5))
NMI_dat_mean <- NMI_dat %>% group_by(benchmark, method) %>% 
...
F1_dat$method <- F1_dat$method %>% factor(levels = c("LRAcluster", "SNF", "CIMLR", "Mocluster",
                                                     "intNMF", "MCIA", "iClusterPlus", "PINSPlus"))
F1_dat$benchmark <- F1_dat$benchmark %>% factor(levels = paste0("Benchmark", 1:5))
F1_dat_mean <- F1_dat %>% group_by(benchmark, method) %>% 
  summarise(mean = mean(F1)) %>% data.frame() %>% mutate(Index = "F1-score")

library(ggplot2)
library(ggalt)

Clust_data <- rbind(ARI_dat_mean, NMI_dat_mean, F1_dat_mean) %>% 
  mutate(Index = factor(Index, levels = c("ARI", "NMI", "F1-score")))

Clust_data_wide <- Clust_data %>% mutate(mean = sprintf("%.3f", mean) %>% as.numeric)
Clust_data_wide <- Clust_data_wide %>% pivot_wider(names_from = method, values_from = mean)

g_clust_line <- Clust_data %>% ggplot(aes(x = benchmark, y = mean, group = method, color = method, shape = method)) +
  geom_point(size = 2.5) + 
  geom_line(linewidth = 1) + 
  scale_color_manual(values  = MetBrewer::met.brewer("VanGogh2", 8)) +
  scale_shape_manual(values = c(1:8)) + 
  scale_x_discrete(labels = as.character(seq(2, 6, 1))) + 
  facet_wrap(. ~ Index, ncol = 3, labeller = label_wrap_gen(multi_line = T)) +
  theme_bw() + xlab("Number of subgroups") + ylab("") + 
