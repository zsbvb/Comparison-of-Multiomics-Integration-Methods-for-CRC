...
pathDat <- R.utils::Arguments$getWritablePath("Data/SimDat_Delta")
pathMeth <- R.utils::Arguments$getWritablePath("Output/SimDat_Delta")

S <- 50

n.sample <- c(rep(150, 7))
...
p.DMP <- c(rep(0.1, 7))
delta.methyl <- c(seq(0, 3, 0.5))
delta.expr <- c(seq(0, 3, 0.5))
delta.protein <- c(seq(0, 3, 0.5))
noise.methyl <- c(rep(FALSE, 7))
noise.expr <- c(rep(FALSE, 7))
noise.protein <- c(rep(FALSE, 7))
...
for(ii in seq(along = listBenchmark)){
  ...
  print("PINSPlus")
  PINSPlusresults <- mclapply(data, IntMultiOmics, method = "PINSPlus", K = K, mc.cores = nbCPU)
...
ARI_dat$benchmark <- ARI_dat$benchmark %>% factor(levels = paste0("Benchmark", 1:7))
...
NMI_dat$method <- NMI_dat$method %>% factor(levels = c("LRAcluster", "SNF", "CIMLR", "Mocluster",
                                                       "intNMF", "MCIA", "iClusterPlus", "PINSPlus"))
NMI_dat$benchmark <- NMI_dat$benchmark %>% factor(levels = paste0("Benchmark", 1:7))
...
F1_dat$method <- F1_dat$method %>% factor(levels = c("LRAcluster", "SNF", "CIMLR", "Mocluster",
                                                     "intNMF", "MCIA", "iClusterPlus", "PINSPlus"))
F1_dat$benchmark <- F1_dat$benchmark %>% factor(levels = paste0("Benchmark", 1:7))
...
Delta_data <- rbind(ARI_dat_mean, NMI_dat_mean, F1_dat_mean) %>% 
  mutate(Index = factor(Index, levels = c("ARI", "NMI", "F1-score")))

Delta_data_wide <- Delta_data %>% mutate(mean = sprintf("%.3f", mean) %>% as.numeric)
Delta_data_wide <- Delta_data_wide %>% pivot_wider(names_from = method, values_from = mean)

g_delta_line <- Delta_data %>% ggplot(aes(x = benchmark, y = mean, group = method, color = method, shape = method)) +
  geom_point(size = 2.5) + 
  geom_line(linewidth = 1) + 
  scale_color_manual(values  = MetBrewer::met.brewer("VanGogh2", 8)) +
  scale_shape_manual(values = c(1:8)) + 
  scale_x_discrete(labels = as.character(seq(0, 3, 0.5))) + 
  facet_wrap(. ~ Index, ncol = 3, labeller = label_wrap_gen(multi_line = T)) +
  theme_bw() + xlab("Shift") + ylab("") + 
